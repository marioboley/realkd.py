...
operator_bound = {"<=": 1, "<": 1, ">=": 0, ">": 0}  # upper bound

operator_comp = {"<=": min, "<": min, ">=": max, ">": max}  # upper bound


def patch_from_rule(
    rule, fc="blue", axes={"x1": 0, "x2": 1}, x_min=-4, x_max=4, y_min=-4, y_max=4
):
    ...
    return Rectangle(
        (x, y), dx, dy, fill=True, color="black", lw=1, ls="-", fc=fc, alpha=0.2
    )
...
def rf_rule_as_query(self):
    ...
    for cond in self.conditions:
        ...
        props.append(
            KeyValueProposition(cond.feature_index, cond.feature_name, constraint)
        )
